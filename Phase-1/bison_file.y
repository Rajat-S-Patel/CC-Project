...
%token ID CLASS NUM T_lt T_gt COMMA STRC TERMINATOR RETURN FLT T_lteq T_gteq T_neq T_eqeq T_pl T_min T_mul T_div T_and T_or T_incr T_decr T_not T_eq WHILE INT CHAR FLOAT VOID H MAINTOK INCLUDE BREAK  CONTINUE IF ELSE COUT STRING FOR OB CB OBR CBR ENDL CHARACTER str_ins str_extr CIN QMARK COLON
%left T_pl T_min
%left T_mul T_div 
...
START
    : INCLUDE T_lt H T_gt CLASS_DEC FunctionDeclaration
    | INCLUDE T_lt H T_gt FunctionDeclaration
    | INCLUDE '\"' H '\"' MAIN
    | MAIN
    ;

FunctionDeclaration : FunctionName OB CB TERMINATOR FunctionDeclaration	  
		   | FunctionName OB ParamsType CB TERMINATOR FunctionDeclaration
		   | MAIN
               | Function MAIN
		   ;
...
ParamsType : TYPE	ID COMMA ParamsType {
                  if(!insert(&count, t_scope, $1, $2, yylineno)){
                        yyerror("Variable redeclared");
                  }
            }
	   | TYPE COMMA ParamsType
         | TYPE ID{
               if(!insert(&count, t_scope, $1, $2, yylineno)){
                        yyerror("Variable redeclared");
                  }
...
Function : 
	 | FunctionName OB CB BODY
       | FunctionName OB ParamsType CB BODY 
       | 
	 ;
...
COND_STMT
            ...
            | IF OB COND CB LOOPBODY ELSE IF OB COND CB LOOPBODY
            | IF OB COND CB LOOPBODY ELSE IF OB COND CB LOOPBODY ELSE LOOPBODY
            | IF OB COND CB LOOPBODY ELSE IF OB COND CB LOOPBODY ELSE IF OB COND CB LOOPBODY
            | IF OB COND CB LOOPBODY ELSE IF OB COND CB LOOPBODY ELSE IF OB COND CB LOOPBODY ELSE LOOPBODY
            ;
LOOPBODY
	  : OBR C CBR
	  | TERMINATOR
...
TERNARY_EXPR
      : OB COND CB QMARK statement COLON statement 
      ;
...
PRINT
      : COUT str_ins ARITH_EXPR PRINT_REP
      | COUT str_ins STRING   PRINT_REP

PRINT_REP
            : str_ins ARITH_EXPR PRINT_REP
            | str_ins STRING PRINT_REP
            | 
            | ENDL PRINT_REP
            ;
